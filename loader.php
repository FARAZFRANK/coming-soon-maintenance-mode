...
if ( 'solid' === $csmm_bg_type ) {
	...
	$pattern_css_map = array(
		...
		'sakura'    => "background-color: #1a0b18 !important; background-image: radial-gradient(circle at 50% 50%, rgba(244, 114, 182, 0.45) 10%, transparent 20%), radial-gradient(circle at 20% 20%, rgba(251, 113, 133, 0.4) 15%, transparent 25%), radial-gradient(circle at 80% 80%, rgba(236, 72, 153, 0.4) 15%, transparent 25%), radial-gradient(circle at 70% 30%, rgba(244, 114, 182, 0.35) 12%, transparent 24%) !important; background-size: 100px 100px !important; background-repeat: repeat !important;",
	);
	$pat_rule = isset( $pattern_css_map[ $csmm_bg_pattern ] ) ? $pattern_css_map[ $csmm_bg_pattern ] : $pattern_css_map['waves'];
	$dynamic_css .= "body, html, .s-home, main.s-home, section.s-home, #home, .template-one, .template-two, .prospectus-card, .main-card, .split-layout, #particles-js, .home-particles, #bg, .bg-image, .bg-container { {$pat_rule} background-attachment: fixed !important; }\n";
