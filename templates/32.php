...
<head>
    ...
    <!-- CSS
    ================================================== -->
    <link rel="stylesheet" href="<?php echo esc_url( CSMM_URL.'templates/css/32.css' ); ?>">
...
<body>
    ...
    <main class="min-h-screen flex items-center justify-center p-4">
        <div class="main-content w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
            
            <!-- Left Side: Cover Art -->
            <div class="w-full max-w-sm mx-auto">
                <div class="aspect-square bg-gray-800 rounded-lg cover-art flex items-center justify-center p-8">
                    <!-- Placeholder for Podcast Cover Art -->
                    <svg class="w-2/3 h-2/3 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"></path></svg>
                </div>
            </div>

            <!-- Right Side: Content -->
            <div class="text-center md:text-left">
                <p class="font-semibold text-violet-400 mb-2"><?php esc_html_e( "A New Podcast", 'coming-soon-maintenance-mode' ); ?></p>
                <h1 class="title-font text-5xl md:text-6xl text-white mb-4">
                    <?php if($csmm_title != "") { echo esc_html( $csmm_title ); } ?>
                </h1>
                <p class="podcast-desc text-lg text-gray-400 max-w-xl leading-relaxed" style="margin-top: 1rem !important; margin-bottom: 3rem !important; line-height: 1.8 !important;">
                    <?php if($csmm_description != "") { echo esc_textarea( stripslashes( $csmm_description ) ); } ?>
                </p>

                <!-- Countdown Timer -->
                <div id="countdown" class="grid grid-cols-4 gap-4 max-w-sm mx-auto md:mx-0" style="margin-top: 1.5rem !important; margin-bottom: 2.75rem !important;">
                    <div>
                ...
                <div class="w-full mb-10">
                    <p class="mb-3 font-semibold text-gray-300"><?php esc_html_e( "Subscribe to be notified on launch day.", 'coming-soon-maintenance-mode' ); ?></p>
                    <form id="subscribe-form" method="post" class="flex flex-col sm:flex-row gap-3">
                        <input type="email" id="csmm-email" name="csmm-email" placeholder="Enter your email" class="subscribe-input flex-grow p-3 rounded-md focus:outline-none text-center sm:text-left" required>
                        <input type="hidden" id="csmm-email-nonce" name="csmm-email-nonce" value="<?php echo esc_attr( wp_create_nonce( 'csmm-email-nonce' ) ); ?>">
						<button type="submit" class="subscribe-btn py-3 px-6 rounded-md"><?php esc_html_e( "Subscribe", 'coming-soon-maintenance-mode' ); ?></button>
                    </form>
					<?php
					$csmm_flag = 0;
    ...
    <script>		
		...
		const countdown = () => {
		  ...
		  if (distance < 0) {
                ...
                document.getElementById('countdown').innerHTML = '<div class="col-span-4 text-2xl font-bold text-center md:text-left w-full">First Episode Out Now!</div>';
                return;
            }
